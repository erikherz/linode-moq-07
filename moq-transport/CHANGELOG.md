...
- Fix finish() recursion: web_transport::SendStream has no inherent finish()
- Fix infinite recursion in web_transport_finish causing stack overflow
- Use spawn_blocking for subgroup tasks to get larger stack
- Box async futures to reduce state machine depth during drop
- Add global drop depth tracking to debug stack overflow
- Remove task abort() calls to prevent stack overflow
